...
use crate::tuple::{normalize, point, Tuple, vector};
...
fn tick(env: &Environment, pro: Projectile, canvas: &mut Canvas) -> Projectile {
    ...
    let w = pro.position.x as usize;
    let h = canvas.height - pro.position.y as usize;
...
fn main() {
    ...
    while projectile.position.y > 0 as f64 {
